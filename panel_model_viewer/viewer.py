...
class ModelViewer(JSComponent):
    ...
    src = param.ClassSelector(class_=(str, bytes, Path), doc="""
    ...
    export function render({ model, el }) {
      ...
      model.on('src', () => { viewer.src = model.src; });
      ...
      model.on('poster', () => { viewer.poster = model.poster; });
       model.on('style', () => {
        for (const [key, value] of Object.entries(model.style)) {
            viewer.style[key] = value;
        }
    ...
    def _get_resources(self):
        ...
        return {
            'js': {
                'model-viewer': 'static/model-viewer.min.js'
            }
        }
